...
import threading, time, traceback, os, socket, sys, struct
            ...
            try:
                ...
                if self.state == 1 :
                    ...
                    if( time >= self.timestamp):
                        ...
                        x = struct.unpack("d", self.connection.recv(8))
                        y = struct.unpack("d", self.connection.recv(8))
                        xstr = '%.5f' % x
                        ystr = '%.5f' % y
                        ...
                        angle = 0
                        while (angle < 360):
                            distance = struct.unpack("d", self.connection.recv(8))
                            self.lidar_data[angle] = distance, 10
                            angle += 1
